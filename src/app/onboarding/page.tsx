...
import { z } from 'zod';
import {
  generateOnboardingGuide as runGenerateOnboardingGuide,
  type GenerateOnboardingGuideOutput,
} from '@/ai/flows/generate-onboarding-guides';
...
const formSchema = z.object({
  ...
  projects: z
    .string()
    .min(2, { message: 'Projects must be at least 2 characters.' }),
...
export default function OnboardingPage() {
  ...
  const onDrop = useCallback(
    (acceptedFiles: File[], fileRejections: any[]) => {
      ...
      if (fileRejections.length > 0) {
        setFileError(
          'File is too large or not a valid type. Please upload a supported file under 2MB.'
        );
        setFileName('');
  ...
  async function onGuideSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      const result = await runGenerateOnboardingGuide({
        ...
        internalDocumentation:
          values.internalDocumentation || 'No specific documentation provided.',
      });
      ...
      setError(
        'An error occurred while generating the guide. Please try again.'
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleTaskToggle = (sectionIndex: number, taskIndex: number) => {
    ...
    newSections[sectionIndex].tasks[taskIndex].completed =
      !newSections[sectionIndex].tasks[taskIndex].completed;
    ...
    <div className="p-4 md:p-6 space-y-6 h-full flex flex-col">
      <header>
        ...
        <p className="text-muted-foreground">
          Generate a new hire guide and ask questions about it.
        </p>
      </header>
      ...
      <div className="grid md:grid-cols-2 gap-6 flex-1">
        <div className="flex flex-col gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Provide employee details to generate a personalized guide.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form}>
                <form
                  onSubmit={form.handleSubmit(onGuideSubmit)}
                  className="space-y-4"
                >
                  <FormField
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="e.g., Project Phoenix, Q3 initiatives"
                            {...field}
                            disabled={isLoading}
                          />
                        </FormControl>
                  ...
                  <FormItem>
                    ...
                    <div
                      {...getRootProps()}
                      className={cn(
                        'relative w-full p-4 flex flex-col items-center justify-center border-2 border-dashed rounded-lg cursor-pointer transition-colors',
                        isDragActive
                      ...
                      {fileName ? (
                        <div className="text-center">
                          ...
                          <Button
                            ...
                            className="mt-0 text-xs h-auto py-0"
                          >
                            Remove file
                          </Button>
                        </div>
          ...
          {isLoading && (
            <Card className="flex flex-col flex-1 items-center justify-center">
              ...
              <p className="mt-2 text-muted-foreground">
                Generating your interactive guide...
              </p>
            </Card>
          )}
          ...
          {guide && (
            <Card className="flex flex-col flex-1">
              <CardHeader>
                ...
                <CardDescription>
                  An interactive, step-by-step guide for the new hire.
                </CardDescription>
              </CardHeader>
              <CardContent className="flex-1">
                <ScrollArea className="h-full max-h-[400px] pr-4">
                  <Accordion
                    type="multiple"
                    defaultValue={guide.sections.map((s) => s.title)}
                    className="w-full"
                  >
                            ...
                            {section.tasks.map((task, taskIndex) => (
                              ...
                              >
                                <Checkbox
                                  ...
                                  onCheckedChange={() =>
                                    handleTaskToggle(sectionIndex, taskIndex)
                                  }
                                />
                                <label
                                  htmlFor={task.id}
                  ...
                  <Alert className="mt-4">
                    ...
                    <AlertDescription>{guide.progressReport}</AlertDescription>
                  </Alert>
                </ScrollArea>
              </CardContent>
            </Card>
        ...
        <Card className="flex flex-col">
          <CardHeader>
            ...
            <CardDescription>
              Ask the AI assistant about the generated onboarding guide.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex-1 flex flex-col gap-4">
            <div className="flex-1 relative">
              <ScrollArea
                className="h-full pr-4 absolute inset-0"
                  ...
                  {chatMessages.length === 0 && (
                    <div className="flex h-full items-center justify-center rounded-lg border-2 border-dashed border-border text-muted-foreground">
                      <div className="text-center">
                        ...
                        <p className="mt-2">
                          Generate a guide to start chatting.
                        </p>
                      </div>
                    </div>
                  )}
